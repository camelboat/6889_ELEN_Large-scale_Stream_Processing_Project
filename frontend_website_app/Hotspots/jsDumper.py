...
class BigQueryReader:
    def __init__(self, location_dict, file_dir, output_dir):
        ...
        self.credentials, self.project_id = service_account.Credentials.from_service_account_file(
            'streamprocessing-a4b6b9056005.json'), 'streamprocessing'
...
class TweetsServer:
    ...
    def get_tweets_ids(self):
        ...
        for i in range(52):
            print("Getting tweets ID in state No. %d..." % i)
            query = tmp_list[i]
            result = api.search(q=query + ' #COVID19', count=20)["statuses"]
...
def main():
    js_file = "static/us-states.js"
    output_file = "static/tmp-new-us-states.js"
    ...
    with open('static/tmp-new-us-states.js', 'r') as tmp_file:
        tmp_lines = tmp_file.readlines()
    with open('static/new-us-states.js', 'w') as new_file:
        new_file.writelines(tmp_lines)


if __name__ == "__main__":
    cnt = 0
    while True:
        print(">>> Dumper No. %d <<<" % cnt)
        t0 = time.time()
        main()
        cnt += 1
        print("Totally use %f seconds" % (time.time() - t0))
        time.sleep(5)
